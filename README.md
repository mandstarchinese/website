...
```
© 2026 MandstarChinese　|　特定商取引法に基づく表記
```

点这个链接就能进入。5个原有页面 + 这个新页面自己的页脚，都已经加上了这个入口。

### 视觉风格

顶部标题区和其他页面一样，保留背景图和星星装饰，风格与全站统一；正文是一个"项目 / 内容"两栏表格，和网站其他表格用的是同一套配色逻辑，手机端会自动变成"项目在上、内容在下"的堆叠布局，方便阅读。

### "キャンセル・返金について" 和 "レッスン日時の変更について" 支持PDF附件形式

这两项内容通常比较长（要分别写清楚录播课/一对一课程/体验课的规则），如果不想把大段文字直接写进表格，可以改成"一句话 + PDF附件链接"的形式，网站已经按这个思路做好了：

1. 准备好对应的规则文档，导出成 PDF
2. 文件分别重命名为 `cancellation-policy.pdf`（取消退款政策）和 `reschedule-policy.pdf`（改期政策）
3. 在网站根目录新建一个 `docs` 文件夹（如果还没有），把两个 PDF 放进去
4. 刷新网页，页面上「📎 キャンセル・返金ポリシー（PDF）」「📎 レッスン日時変更ポリシー（PDF）」这两个按钮点击后就会打开你的 PDF

**注意**：和照片/视频不同，这两个链接**不会自动检测文件是否存在**（纯静态网站做不到这一点），所以在你放入真实 PDF 之前，点击按钮会显示"文件不存在"。如果你更想直接把文字规则写在表格里、不想用附件形式，打开 `tokushoho.html` 找到这两行，把"詳細は以下のファイルをご確認ください。"和下面的 `<div class="doc-attach">...</div>` 整段删掉，把黄色高亮的 `<span class="todo-fill">` 部分换成你的文字说明即可（做法和其他黄色高亮项一样）。
