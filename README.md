...
├── tokushoho.html        特定商取引法に基づく表記（法律要求的信息披露页，详见第十节，仅在页脚有入口，不在主导航栏）
├── riyokiyaku.html       利用規約（服务条款页，详见第十二节，仅在页脚有入口，不在主导航栏）
├── css/
...
├── docs/                  （需要自己新建）放 cancellation-policy.pdf、reschedule-policy.pdf，特定商取引法页用的PDF附件，见第十节说明
...
| 首页 index.html | `photo-about.jpg`（可选，备用封面图） | 16:9 | "MandstarChineseについて"旁边视频区域的备用封面图。正常情况下网站会自动截取视频本身的画面作为封面，不需要单独准备这张图；只有当自动截取失败时才会用到它 |
...
**封面图是自动的，不需要单独准备**：网站会自动读取视频本身的一帧画面作为封面（播放前显示的静止画面），不会出现"黑底+播放键"的情况。如果想手动指定封面图，把图片放进 `images/photo-about.jpg`，会在自动读取失败时作为备用显示。

> 注意：这个自动读取封面的功能，在**本地双击 `index.html` 预览**时，出于浏览器安全限制，有可能不生效（仍会显示黑底或备用封面图）；上传到 GitHub Pages 正式网址后就会正常显示，属于本地预览限定的小差异，不影响线上效果。
...
## 十、新增页面：特定商取引法に基づく表記（tokushoho.html）

日本法律（特定商取引法第11条）要求，任何面向日本消费者销售商品/服务的网站，都必须有一个页面公开披露经营者信息、价格、付款方式、退款规则等。这是新增的 `tokushoho.html` 页面，专门用来满足这个要求。

### 入口放在哪里

这个页面**不放进顶部主导航栏**（这是日本网站的通行做法——法律声明页不算营销内容，不占导航位置），只在**每个页面页脚最下面一行、版权信息旁边**放了一个小链接：

```
© 2026 MandstarChinese　|　利用規約　|　特定商取引法に基づく表記
```

点这个链接就能进入。5个原有页面 + `tokushoho.html` + `riyokiyaku.html` 自己的页脚，都已经加上了这两个入口。

### 视觉风格

顶部标题区和其他页面一样，保留背景图和星星装饰，风格与全站统一；正文是一个"项目 / 内容"两栏表格，和网站其他表格用的是同一套配色逻辑，手机端会自动变成"项目在上、内容在下"的堆叠布局，方便阅读。

### "キャンセルポリシー"“返金ポリシー” 和 "レッスン日時の変更について" 支持PDF附件形式

这三项内容通常比较长（要分别写清楚录播课/一对一课程/体验课的规则），如果不想把大段文字直接写进表格，可以改成"一句话 + PDF附件链接"的形式，网站已经按这个思路做好了：

1. 准备好对应的规则文档，分别导出成 PDF
2. 文件分别重命名为 `cancellation-policy.pdf`（取消政策）、`refund-policy.pdf`（退款政策）、`reschedule-policy.pdf`（改期政策）
3. 在网站根目录新建一个 `docs` 文件夹（如果还没有），把三个 PDF 放进去
4. 刷新网页，页面上对应的「📎 ○○ポリシー（PDF）」按钮点击后就会打开你的 PDF

**注意**：和照片/视频不同，这几个链接**不会自动检测文件是否存在**（纯静态网站做不到这一点），所以在你放入真实 PDF 之前，点击按钮会显示"文件不存在"。如果你更想直接把文字规则写在表格里、不想用附件形式，打开 `tokushoho.html` 找到对应的行，把"詳細は以下のファイルをご確認ください。"和下面的 `<div class="doc-attach">...</div>` 整段删掉，把黄色高亮的 `<span class="todo-fill">` 部分换成你的文字说明即可（做法和其他黄色高亮项一样）。

### ⚠️ 必须做的事：替换黄色高亮的占位内容

打开 `tokushoho.html`，你会看到一部分内容带有**黄色底色高亮**，比如"中国公司的正式登记名称"这种——这些都是从你上传的文件里摘出来的**待填写提示**，不是可以直接上线的正式内容。

替换方法：找到形如 `<span class="todo-fill">这里的提示文字</span>` 的地方，把提示文字换成真实信息，然后把整段 `<span class="todo-fill">` 和对应的 `</span>` 标签删掉（只留中间替换后的文字），黄色高亮就会消失，变成正常的黑色文字。例如：

```html
<!-- 替换前 -->
<td><span class="todo-fill">负责人真实姓名</span></td>

<!-- 替换后 -->
<td>張 三</td>
```

**这一步比补照片更重要——在所有黄色高亮内容替换成真实信息之前，不建议正式上线这个页面**（其余页面不受影响，可以正常上线；只是页脚会多一个还没填完的法律声明链接）。

## 十二、新增页面：利用規約（riyokiyaku.html）

按日本网站惯例新增的服务条款页，采用"第◯条"逐条列示格式。**内容已替换成你上传的正式《利用規約 Terms of Service》文件里的完整20条条款**（适用范围、服务内容、预约、付款、变更/取消、通信环境、讲师、禁止事项、知识产权、个人信息、服务暂停、免责、反社会势力排除、准据法、管辖法院等），末尾还加上了联系方式表格和附则生效日期，都是你文件里的原文，没有改动措辞。

**入口位置**：和特定商取引法页一样，只放在页脚版权信息旁边，不占主导航栏。

**做了2处小优化**（不影响原文内容）：
- 第5条付款条款里的"返金ポリシー"、第6条"レッスン日時変更ポリシー"、第7条"キャンセルポリシー"这三处，加上了指向 `tokushoho.html` 对应条目的超链接，方便用户点击直接查看具体规则，不用跳出条款重新找。
- 页面没有需要你填写的黄色高亮项——这份内容已经是你确认过的正式文本，无需二次修改。**唯一需要留意的是**：如果以后修改了原始 Word 文件的内容，记得同步更新这个页面（以及页面里的"最終更新日"和文末"附則"日期），保持两边一致。

## 十三、待办事项清单（上线前请确认）

- [ ] 在 `js/config.js` 中填入真实的预约页面 URL
- [ ] 在 `js/config.js` 中填入真实的 YouTube / TikTok / LINE / WeChat / Lemon8 链接
...
- [ ] **在 `tokushoho.html` 中把所有黄色高亮的 `todo-fill` 内容替换成真实的公司信息（见第十节说明，上线前必须完成）**
- [ ] （可选）把取消/退款/改期政策整理成 `cancellation-policy.pdf`、`refund-policy.pdf`、`reschedule-policy.pdf` 放入 `docs` 文件夹
